...
function Upload() {
  ...
  const adddata = () => {
    if (url == "") {
      ...
      setData([{ url: url, status: "pending" }, ...data]);
      setCheckedItems([...checkedItems, { url: url, status: "pending" }]);
      setUrl("");
      setModal(false);
  ...
  const del = (obj) => {
    ...
    if(obj.id){
      ...
      axios
      .delete(`http://3.88.224.113:90/get_prop/${obj.id}/`)
      .then((response) => {
  ...
  const scrapAll = () => {
    if(scrapCheckboxRef.current.checked){
      ...
      setData(updatedArray);
      setCheckedItems([]);
      console.log('else-updatedArray',updatedArray)
    }
  ...
  const next = () => {
    ...
    if (checkedItems.length == 0) {
      ...
      axios
        .post("http://3.88.224.113:90/crawl/", data)
        .then((response) => {
          const hell = checkedItems?.map((item) => item.url);
          console.log("hell", hell);

          axios.post("http://3.88.224.113:90/jobs/", hell).then((response) => {
                      ...
                      <table className="min-w-max w-full table-auto">
                        <thead>
                          <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            ...
                            <th className="py-3 px-6 text-left w-1/5">
                              <span>Scrap</span>
                            </th>
                            <th className="py-3 px-6 text-left w-1/5">
                              Status
                            </th>
                            <th className="py-3 px-6 text-center w-1/5">
                              Actions
                            </th>
                          </tr>
                        </thead>
                              ...
                              <td className="py-3 px-6 text-center">
                                <div className="flex text-center justify-center">
                                  <div className="transform hover:text-purple-500">
                                    <button
                                      onClick={() => del(datas)}
                                      className="group relative p-2 w-24 overflow-hidden rounded-2xl bg-red-500 text-xs font-bold text-white"
                                    >
                                      Delete
