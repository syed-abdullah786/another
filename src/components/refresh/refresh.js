...
function Add({data,setData}) {
  ...
  const handleChange = (val) => {
    ...
    setSelectedvalues(val);}
      ...
      notify22("You can only select one user", "error");
    }
  };
  ...
  const Refresh = () => {
    if (selectedvalues.length == 0) {
      ...
      notify22("Please select dat and time", "error");
    }
    else {
      const ids = selectedvalues.map((item) => item.id);
      const info = {
        refresh_at: moment(value).format("YYYY-MM-DDTHH:mm:ss"),
        status:'pending',
        reality_user:ids
      };
      axios
        .post("http://18.191.139.138:90/refresh/", info)
        .then((response) => {
          setData([...data,response.data])
          console.log('response.data,',response.data)
          notify22("Uploaded", "success");
          setMxmodal(false);
          setSelectedvalues([]);
        })
        .catch((e) => {
    ...
    <>
      ...
      {mxmodal && (
        <div className="py-12 bg-gray-700 bg-opacity-30 transition duration-150 ease-in-out z-10 fixed top-0 right-0 bottom-0 left-0">
          ...
          >
            <div className="relative mx-auto w-full py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
              ...
              />
              <p className="text-left float-left text-gray-800 text-sm mb-2 font-bold leading-tight tracking-normal">
                Select Date
              </p>
              <DateTimePicker
                onChange={(e) => {
                ...
                calendarClassName={"border-gray-300 rounded border"}
                className={"text-gray-600 mb-4 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center text-sm border-gray-300 rounded border"}
                dayPlaceholder={"dd"}
                monthPlaceholder={"mm"}
      ...
      {modal && (
        <div className="py-12 bg-gray-700 bg-opacity-30 transition duration-150 ease-in-out z-10 fixed top-0 right-0 bottom-0 left-0">
          <div
            ...
            className="container mx-auto w-11/12 md:w-2/3 max-w-lg"
          >
            <div className="relative py-8 mx-auto w-full px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
              <h1 className="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">
